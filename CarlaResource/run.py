...
PORT = 2000
DATASET_NUM = 1
MAX_FRAMES = 500000000000 #max frmaes for each dataset
# imgae must be small !!
IMG_LENGTH = 600
IMG_WIDTH = 400
FOV = 90 # degree
MAX_SPEED = 40 # km/h, slow down for better ctrl !
TIMEOUT = 10.0 # s, connection timeout
CAMERA_TRANS = carla.Transform(
        #location = carla.Location(x=1.5, y=0.0, z=2.4),
        location = carla.Location(x=-6, y=0.0, z=3),
        rotation = carla.Rotation(pitch=-10, yaw=0.0, roll=0.0))
...
def get_rgb_img(image):
    global actor_list, world, control
    ...
    with open(OUTPUT_PATH + '/location.txt', 'a') as file:
        file.write(str(image.frame_number) + '\t'
                 ...
                 + str(control.steer) + '\t'
                 + str(control.brake) + '\t'
                 + str(control.hand_brake) + '\t'
                 + str(control.reverse) + '\t'
                 + str(control.manual_gear_shift) + '\t'
                 + str(control.gear) + '\t'
                 + '\n')
